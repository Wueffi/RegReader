                        ...
                        .then(argument("state", StringArgumentType.word())
                                ...
                                .executes(context -> {
                                    ...
                                    if (state.equalsIgnoreCase("toggle")) {
                                        ...
                                        context.getSource().sendError(Component.nullToEmpty("Invalid argument!"));
                                        return 0;
                                    }

                                    context.getSource().sendFeedback(Component.nullToEmpty("All HUD's are now " + (RegReaderConfig.isHudEnabled() ? "enabled" : "disabled")));
                                    return 1;
                                }))
                        .then(literal("color")
                                    ...
                                    .then(argument("color", StringArgumentType.string())
                                            .executes(context -> {
                                                ...
                                                if (color.matches("^#[0-9A-Fa-f]{8}$")) {
                                                    ...
                                                    if(HUD == null) {
                                                        context.getSource().sendError(Component.nullToEmpty("HUD " + hud + " not found."));
                                                        return 0;
                                                    }
                                                    HUD.setHUDColor(color);
                                                    context.getSource().sendFeedback(Component.nullToEmpty(hud + " color set to " + color));
                                                } else {
                                                    context.getSource().sendError(Component.nullToEmpty("Invalid color format. Please use a valid hex color code (#AARRGGBB). Remember to use '"));
                                                }
                                                return 1;
                                            })
                                    )
                                ...
                                .then(argument("name", StringArgumentType.string())
                                        .executes(context -> {
                                            ...
                                            if (!name.isEmpty()) {
                                                ...
                                                context.getSource().sendFeedback(Component.nullToEmpty("Added HUD: " + name));
                                            } else {
                                                context.getSource().sendError(Component.nullToEmpty("Name can not be empty!"));
                                            }
                                            return 1;
                                        })
                                )
                                ...
                                .then(argument("name", StringArgumentType.string())
                                        ...
                                        .executes(context -> {
                                            ...
                                            if (!name.isEmpty()) {
                                                ...
                                                context.getSource().sendFeedback(Component.nullToEmpty("Removed HUD: " + name));
                                            } else {
                                                context.getSource().sendError(Component.nullToEmpty("Name can not be empty!"));
                                            }
                                            return 1;
                                        ...
                                        .then(argument("newName", StringArgumentType.string())
                                            .executes(context -> {
                                                ...
                                                if (name.isEmpty() || newName.isEmpty()) {
                                                    context.getSource().sendError(Component.nullToEmpty("Name and new Name can not be empty!"));
                                                    return 0;
                                                }
                                                if(!HUDManager.renameHUD(name, newName)) {
                                                    context.getSource().sendError(Component.nullToEmpty("Could not rename HUD."));
                                                    return 0;
                                                }
                                                context.getSource().sendFeedback(Component.nullToEmpty("Renamed HUD: " + name + " to: " + newName));
                                                return 1;
                                            })
                                        )
                                )
                                    ...
                                    .then(argument("base", integer(2, 16))
                                            ...
                                            .executes(context -> {
                                                ...
                                                if (validBases.contains(base)) {
                                                    ...
                                                    context.getSource().sendFeedback(Component.nullToEmpty(hud + " base set to " + base));
                                                    return 1;
                                                } else {
                                                    context.getSource().sendError(Component.nullToEmpty("Invalid base! Use 2, 8, 10, or 16."));
                                                    return 0;
                                                }
                                            })
                                    )
                                ...
                                .then(argument("HUD", StringArgumentType.string())
                                    ...
                                    .then(argument("Boolean", BoolArgumentType.bool())
                                            .executes(context -> {
                                                ...
                                                context.getSource().sendFeedback(Component.nullToEmpty(hud + " Colored names set to " + setting));
                                                return 1;
                                            })
                                    )
                                )
                            ...
                            .then(argument("HUD", StringArgumentType.string())
                                    ...
                                    .then(argument("Width", IntegerArgumentType.integer())
                                            .executes(context -> {
                                                ...
                                                context.getSource().sendFeedback(Component.nullToEmpty(hud + " Width set to " + Width));
                                                return 1;
                                            })
                                    )
                            )
                                    ...
                                    .then(literal("X")
                                            .then(argument("XPos", IntegerArgumentType.integer())
                                                    .executes(context -> {
                                                        ...
                                                        HUD.setxPos(X);
                                                        context.getSource().sendFeedback(Component.nullToEmpty(hud + " X-Position set to " + X));
                                                        return 1;
                                                    })
                                            )
                                    )
                                    .then(literal("Y")
                                            .then(argument("YPos", IntegerArgumentType.integer())
                                                    .executes(context -> {
                                                        ...
                                                        HUD.setyPos(Y);
                                                        context.getSource().sendFeedback(Component.nullToEmpty(hud + " Y-Position set to " + Y));
                                                        return 1;
                                                    })
                                            )
                                    )
